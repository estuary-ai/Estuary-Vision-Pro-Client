...
namespace UnityEngine.XR.VisionOS
{
    ...
    // #if UNITY_IOS && !UNITY_EDITOR
        ...
        void BreakupMesh(MeshFilter meshFilter)
        {
            ...
            {
                Debug.Log("[FRACK YOU] Mesh subsystem not found");
                return;
            }
            ...
            {
                Debug.Log("[FRACK YOU] Face classifications not created");
                return;
            }
            ...
            using (faceClassifications)
            {
                ...
                {
                    Debug.Log("[FRACK YOU] No face classifications");
                    return;
                }
                ...
                meshFilters[(int) ARMeshClassification.Floor] =
                    (m_FloorMeshPrefab == null) ? null : InstantiateFloorPrefab(parent);
        ...
        private MeshFilter InstantiateFloorPrefab(Transform parent)
        {
            Debug.Log("[FRACK YOU] Instantiate Floor Prefab");
            return Instantiate(m_FloorMeshPrefab, parent);
        }
        ...
        void UpdateMesh(MeshFilter meshFilter)
        {
            ...
            XRMeshSubsystem meshSubsystem = m_MeshManager.subsystem;
            if (meshSubsystem == null)
            {
                return;
            }
            ...
            var meshId = ExtractTrackableId(meshFilter.name);
            Debug.Log("[FRACK YOU] Mesh ID: " + meshId.ToString());
            Debug.Log("[FRACK YOU] Allocator.Persistent: " + Allocator.Persistent.ToString());
            var faceClassifications = meshSubsystem.GetFaceClassifications(meshId, Allocator.Persistent);
